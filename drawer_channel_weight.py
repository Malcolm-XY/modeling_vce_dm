...
def get_channel_importances(folder='channel_importances', excel='channel_importances_LD.xlsx', sheet='label_driven_mi_1_5'):
    # define path
    ...
    importances = pd.read_excel(path_excel, sheet_name=sheet, engine='openpyxl')
    importances = importances['ams']
...
def draw_importance_map_from_file(ranking_method='label_driven_mi_1_5', offset=0, transformation='log', reverse=False):
    # 获取数据
    importances = get_channel_importances(sheet=ranking_method)
    if reverse:
...
if __name__ == '__main__':
    ...
    channel_importances = get_channel_importances(sheet='label_driven_mi_1_5')
    draw_importance_map_from_data(channel_importances)
    channel_importances = get_channel_importances(sheet='label_driven_mi_10_15')
    draw_importance_map_from_data(channel_importances)
